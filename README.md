...
1. Start compose using the `compose.yml` file from the root directory of this project with `docker compose up -d`
2. Once the build is done and compose is up and healthy; check using `docker compose ls -a`
3. Now open the PDF in a viewer that supports reloading new file when there are changes like Skim on MacOS, VS Code: Use the "LaTeX Workshop" extension, Linux: Zathura or Okular.
